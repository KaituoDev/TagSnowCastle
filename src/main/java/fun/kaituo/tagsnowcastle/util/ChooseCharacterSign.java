...
import static fun.kaituo.tagsnowcastle.util.Misc.getCharacterDisplayName;
...
public class ChooseCharacterSign extends AbstractSignListener {
    ...
    public ChooseCharacterSign(JavaPlugin plugin, Location location, Class<? extends PlayerData> characterClass) {
        ...
        team = TagSnowCastle.inst().getTagBoard().registerNewTeam(characterClass.getSimpleName());
        team.setColor(color);
    }
    ...
    public void onRightClick(PlayerInteractEvent e) {
        ...
        if (TagSnowCastle.inst().playerCharacterChoices.get(chooser.getUniqueId()).equals(characterClass)) {
            return;
        }
        for (Player p : TagSnowCastle.inst().getPlayers()) {
            p.sendMessage(
        ...
        chooser.sendMessage(
                color + displayName +
                "§f" + "：" + chooseMessage);
        TagSnowCastle.inst().playerCharacterChoices.put(chooser.getUniqueId(), characterClass);
