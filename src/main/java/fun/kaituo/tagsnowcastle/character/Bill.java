...
public class Bill extends Human {
    ...
    public Bill(Player p) {
        ...
        bento = TagSnowCastle.inst().getItem("BillBento");
        serpentGodBlood = TagSnowCastle.inst().getItem("SerpentGodBlood");
    }
    ...
    public boolean castSkill() {
        ...
        for (Player target : HuntState.INST.getHumans()) {
            PlayerData data = TagSnowCastle.inst().idDataMap.get(target.getUniqueId());
            assert data != null;
